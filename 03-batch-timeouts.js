...
***********************************************************************/
//Accepts an array of callback functions and an array of delays in milliseconds.
function batchTimeouts(callbacks, delays) {
  //Initializes an empty array for the Timeout objects.
  let timeoutArray = [];
  //Uses a 'for' loop to "match" each callback within callbacks with its corresponding delay within delays.
  for (let i = 0; i < callbacks.length; i++) {
    //Pushes the Timeout object created from calling setTimeout on each callback-delay pair to timeoutArray.
    timeoutArray.push(setTimeout(callbacks[i], delays[i]));
  }
  //Returns the completed array of Timeout objects.
  return timeoutArray;
}
